...
def load_and_evaluate_model(model_path, data_dir, device, threshold=0.50,run_index=1):
    ...
    test_dataset = TwoChannelEEGDataset(data_dir, run_number=run_index, window_size=1024, overlap=0.7)
...
def plot_results(labels, preds, probs, save_path='test_results.png'):
    ...
    axes[0].set_title("Confusion Matrix - Test Results")
    ...
    axes[1].set_title("ROC Curve - Test Results")
    axes[1].legend()
...
if __name__ == "__main__":
    ...
    # Configuration
    model_path = 'best_fists_model.pth'  # Path to saved model weights
    #data_dir = "Muse_data_OpenCloseFeet_segmented"  # Path to data directory
    #for session1
    # data_dir = r"D:\Faculty\ColumbiaUniversity\dataprocess\EEG\dataProcessing\Interaxon\museS\LSL\Python\muse-lsl-python\Data\sub-EB-43\Muse_data_OpenCloseFistsFeet_session1_segmented"
    # run_index=1
    #for session2
    data_dir=r"D:\Faculty\ColumbiaUniversity\dataprocess\EEG\dataProcessing\Interaxon\museS\LSL\Python\muse-lsl-python\Data\sub-EB-43\Muse_data_OpenCloseFists_session2_segmented"
    run_index=1
    #run_index=2

    try:
        # Load model and evaluate
        labels, preds, probs = load_and_evaluate_model(model_path, data_dir, device,threshold=0.50,run_index=run_index)
        ...
        auc_score = plot_results(labels, preds, probs, save_path='test_fists_results.png')
        ...
        print(f"Open/Close Fist samples: {np.sum(labels == 1)} ({np.mean(labels == 1)*100:.1f}%)")
        
        print(f"\nPrediction Statistics:")
        print(f"Predicted Resting: {np.sum(preds == 0)} ({np.mean(preds == 0)*100:.1f}%)")
        print(f"Predicted Open/Close Fist: {np.sum(preds == 1)} ({np.mean(preds == 1)*100:.1f}%)")
        
    except FileNotFoundError as e:
        print(f"Error: {e}")
        print("Make sure you have:")
        print("1. Trained the model using the original script")
        print("2. The 'best_fist_model.pth' file exists in the current directory")
